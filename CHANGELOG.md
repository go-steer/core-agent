...
- GKE recipe: new `deploy/components/otel/` kustomize component + `deploy/overlays/example-otel/` overlay that wire the daemon to [GKE Managed OpenTelemetry](https://docs.cloud.google.com/kubernetes-engine/docs/how-to/managed-otel-gke). Component patches the daemon's `env:` with the standard OTel SDK vars (`OTEL_TRACES_EXPORTER`, `OTEL_EXPORTER_OTLP_ENDPOINT`, `OTEL_SERVICE_NAME`, `OTEL_RESOURCE_ATTRIBUTES`) targeting the in-cluster managed collector; overlay pins to v2.7.0-dev.4 (first release carrying the env override). Spans land in Cloud Trace with no self-hosted collector to deploy. New site docs page `docs/reference/otel.md` documents the span tree (`mcp.tool_call → digest.process → subagent.llm_call`), attributes, and common queries. See the component README for GKE prerequisites (cluster-wide `--managed-otel-scope`, `roles/cloudtrace.user` IAM) and for non-GKE / self-managed collector usage.
