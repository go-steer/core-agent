...
#### Changed
- **BREAKING:** module path renamed `github.com/go-steer/core-agent` → `github.com/go-steer/core-agent/v2` to satisfy Go's Semantic Import Versioning rule for major versions ≥ 2. Every v2.x tag before this change (v2.0.0 through v2.7.0-dev.4) fails `go install github.com/go-steer/core-agent/...@vX.Y.Z` with `module path must match major version`. This PR fixes it forward from the tag it lands under; historical tags stay broken (they can't be retroactively rewritten). **Consumer migration**: import paths need `/v2` inserted — `github.com/go-steer/core-agent/pkg/agent` → `github.com/go-steer/core-agent/v2/pkg/agent`. Mechanical `sed` on your consumer tree covers it. Container images (`ghcr.io/go-steer/core-agent:X.Y.Z`) and source builds (`git checkout && go build`) were never affected and remain the recommended path for pre-fix tags. Closes [#206](https://github.com/go-steer/core-agent/issues/206).
...
- GKE recipe: new `deploy/components/otel/` kustomize component + `deploy/overlays/example-otel/` overlay that wire the daemon to [GKE Managed OpenTelemetry](https://docs.cloud.google.com/kubernetes-engine/docs/how-to/managed-otel-gke). Component sets only `OTEL_TRACES_EXPORTER=otlp` (our sole non-standard knob, from PR #315) — endpoint, service name, resource attrs, and sampling all ride standard OTel SDK env vars that ADK-go honors directly. On GKE, an `Instrumentation` CR (shipped in the overlay) auto-injects those standard vars into every Pod in the namespace; off GKE, operators patch them into their own overlay. Same component, portable to any OTel-aware runtime. Overlay pins to v2.7.0-dev.4 (first release carrying the env override). New site docs page `docs/reference/otel.md` documents the span tree (`mcp.tool_call → digest.process → subagent.llm_call`), attributes, and common queries. See the component README for GKE prerequisites (cluster-wide `--managed-otel-scope`, `roles/cloudtrace.user` IAM) and non-GKE recipes. `scripts/setup-wif.sh` now enables `cloudtrace.googleapis.com` and grants `roles/cloudtrace.user` to the daemon KSA — both harmless no-ops when the OTel overlay isn't applied, so operators get the IAM in place unconditionally.
