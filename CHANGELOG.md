...
- Release images: drop `org.opencontainers.image.source` LABEL from Dockerfile (it triggers GHCR to link the package to the source repo and render the repo description, overriding per-image descriptions); moved to a manifest ANNOTATION so Dependabot / docker scout / cosign chains still find the upstream repo via OCI metadata.
- Release images: also explicitly unset `image.source` and `image.url` in `docker/metadata-action`'s `labels:` block. `docker/metadata-action` auto-injects both from the git context, so removing the Dockerfile LABEL alone left them baked back in — GHCR kept treating the images as repo-linked. Empty right-hand sides suppress the auto-injection.
