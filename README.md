...
- **Optional AX adapter** — `extras/ax-agent/` (on the `axplore` branch) packages core-agent as a remote [Agent eXecutor (AX)](https://github.com/google/ax) agent. Multi-agent conversations are brokered by AX's planner; see the worked devil + angel example. Off `main` until AX goes public.
