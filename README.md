...
Current release: **v1.4.0** — Gemini tool-calling optimization plus narrative documentation. New exported `agent.DefaultInstruction` with a parallelism mandate adapted from `google-gemini/gemini-cli`; tool descriptions for `read_file` / `grep` / `bash` rewritten to steer the model toward the structured tools the permission gate covers; a new `read_many_files` built-in for batched reads (gemini-cli-shaped); and a default-model flip to `gemini-3.1-pro-preview-customtools` (a Vertex variant fine-tuned to prefer developer-defined tools over raw shell). Direct probe measurement (`dev/parallel-probe/`): Claude on a code-search task: 17 turns → 4, 89s → 28s. Gemini on the same task: tool choice flipped from 15 bash / 1 grep to 19 grep / 4 bash. Three new narrative doc pages — Why core-agent, User guide, Library guide — so the site is no longer reference-only. v1.3.0 added interrupt machinery (`Agent.Inject`, `StartAutonomous` → `*AutonomousHandle`, ESC-mid-turn + double-Ctrl+C-exits in the REPL). v1.2.0 added dynamic background subagents (`spawn_agent`) + a `RemoteAgentSpawner` seam. v1.1.0 added interactive permissions + Gemini grounding visibility. v1.0.1 patched two `--provider=vertex` regressions. v1.0.0 was the first stable release. Public API is under SemVer; breaking changes go through `v1.X.0` minor bumps with a one-version deprecation period when feasible.

```bash
go get github.com/go-steer/core-agent@v1.4.0
```
