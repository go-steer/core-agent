...
Current release: **v1.2.0** — adds dynamic background subagents the parent's model can spawn at runtime (`spawn_agent` / `list_agents` / `check_agent` / `stop_agent` tools, configurable depth + concurrency + budgets), plus a consumer-pluggable `RemoteAgentSpawner` seam for out-of-process subagents (gRPC, K8s Jobs, Cloud Run). Alerts from background subagents flow back to the parent via a synchronous `OnAlert` hook (inline display) and a pre-turn drain (model context). Permissions inherit wholesale from the parent gate with `[<source>] tool wants to ...` attribution and serialized prompter access. v1.1.0 added interactive permissions + Gemini grounding visibility. v1.0.1 patched two `--provider=vertex` regressions. v1.0.0 was the first stable release. Public API is under SemVer; breaking changes go through `v1.X.0` minor bumps with a one-version deprecation period when feasible.
