...
Current release: **v1.3.0** — interrupt machinery, both programmatic and interactive. New `Agent.Inject(message)` lets any external caller (orchestrator goroutine, HTTP handler, test fixture) queue messages that get prepended to the model's next turn as an `[Inbox]` block. New `agent.StartAutonomous(...)` returns an `*AutonomousHandle` with `Pause / Resume / Stop / Inject / Status / Wait` for programmatic control over a running loop. The bundled CLI's REPL now supports Claude Code-style **ESC mid-turn** (cancels the current turn, preserves conversation, prompts for the next input) and **double-Ctrl+C-exits** (single Ctrl+C interrupts; second within 1s exits). The Scion adapter's stdin is now read in a background goroutine so messages arriving during a long-running tool call no longer block — they queue and land on the next turn. v1.2.0 added dynamic background subagents (`spawn_agent`) + a `RemoteAgentSpawner` seam. v1.1.0 added interactive permissions + Gemini grounding visibility. v1.0.1 patched two `--provider=vertex` regressions. v1.0.0 was the first stable release. Public API is under SemVer; breaking changes go through `v1.X.0` minor bumps with a one-version deprecation period when feasible.
