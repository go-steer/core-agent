...
- Built-in tool catalog covering files (`read_file`, `read_many_files`, `write_file`, `edit_file`, `delete_file`, `stat`, `list_dir`), search (`glob`, `grep`), data + network (`json_query`, `fetch_url`), shell (`bash`), planning (`todo`, opt-in `record_plan`), verification (`wait_and_verify` — bounded, read-only poll-until-condition, no shell required), and interactive prompting (opt-in `ask_user`). Disable the whole suite with `--no-builtin-tools` or specific entries with `--disable-tools=bash,write_file`.
...
- Claude-compatible skills auto-discovered from `.agents/skills/<name>/SKILL.md` (project), `~/.agents/skills/` (portable user), and `~/.core-agent/skills/` (legacy user-global fallback). A skill may declare `requires: [shell, kubectl, gcloud]` in its frontmatter; one whose requirements this runtime can't meet is withheld from the toolset at startup and named on stderr, instead of instructing the model to do something a distroless image cannot do.
...
- **Operator hold** (v2.9+): `/interrupt` cancels the in-flight turn *and parks the loop* — auto-continue stands down, a queued inbox message stays queued, and nothing starts a new turn until the operator says how to proceed (`steer` with a new instruction, `continue` unchanged, or `abandon`). `POST /pause` holds without cancelling anything; `POST /agents/<name>/stop` reaches a runaway background subagent, which interrupting the parent alone never did. State is on `GET /status` and pushed as a `pause` SSE frame, so a second dashboard watching the same session sees the hold too.
- **Remote TUI client** (`core-agent-tui`, separately-versioned binary): thin bubble-tea shell on [`go-steer/core-tui`](https://github.com/go-steer/core-tui) that attaches to any daemon over the attach API — full slash parity with the in-process TUI (`/stats`, `/context`, `/compact`, `/done`, `/subagent`, `/btw`, `/memory`, `/skills`, `/mcp`, `/pricing`, `/perms`, `/allow`, `/deny`, `/reload`), Esc to stop-and-hold (`/pause` holds without cancelling) then type to steer or `/continue` / `/abandon` to release, per-turn cost footer, live tool-approval modals over HTTP, mid-turn `/inject`, session switcher.
...
- `lookout-watch` — watches Kubernetes Events, filters + dedupes, and injects matched incidents into per-incident sessions on a core-agent daemon. Pairs with the bundled triage skill to drive diagnose → fix → verify loops via the GKE MCP, gated by plan-first. The watcher ships from [go-steer/k8s-lookout](https://github.com/go-steer/k8s-lookout) as `ghcr.io/go-steer/lookout` (the recipe in [`examples/gke-troubleshoot-agent/`](./examples/gke-troubleshoot-agent/) pins `v0.26.0`); its entrypoint is `lookout watch`. lookout v0.17.0 retired the `k8s-event-watcher` transition naming it inherited from the retired `ghcr.io/go-steer/k8s-event-watcher` image, so the Kubernetes resources, token Secret, and `lookout_*` metric prefix all read as lookout.
