...
func main() {
	...
	authMode := fs.String("auth", "bearer", "auth strategy for outbound attach requests. 'bearer' (default): send attach token in Authorization: Bearer — the direct-attach path. 'google-oauth' (recommended for Cloud Run IAM): source a Google OAuth2 access token via Application Default Credentials and send it in Authorization; the attach token rides on X-Attach-Token. Works with end-user ADC (gcloud auth application-default login) AND service-account ADC. 'google-id-token': audience-bound variant required by IAP; mints an ID token via idtoken.NewTokenSource — does NOT work with end-user ADC, requires a service-account JSON key or impersonation. For Cloud Run IAM, prefer 'google-oauth'.")
