...
| `11-evals-corpus.sh` | The behavioural evals (#652 harness, #966 corpus): every case under `dev/evals/cases/`, each asking whether the agent *behaved* — found a planted image-pull defect whose location the prompt withholds; kept the subject it was delegated when a discoverable skill told it to audit a different one. Graded off the fixture's own kubectl log, not the transcript, and each paired with a no-access baseline that must score zero. The whole corpus runs even when a case fails, and the leg reports a per-case tally. See `dev/evals/README.md`. Optional: `EVAL_MODEL`, `EVAL_CASE` (narrow to one case path), `EVAL_TIMEOUT` | `ANTHROPIC_VERTEX_PROJECT_ID` + ADC |
