...
| `11-evals-cluster-fact.sh` | The behavioural eval (#652): can the agent find a planted image-pull defect in a fixture cluster whose location the prompt withholds? Graded off the fixture's own kubectl log, not the transcript, and paired with a no-access baseline that must score zero. See `dev/evals/README.md`. Optional: `EVAL_MODEL`, `EVAL_CASE`, `EVAL_TIMEOUT` | `ANTHROPIC_VERTEX_PROJECT_ID` + ADC |
