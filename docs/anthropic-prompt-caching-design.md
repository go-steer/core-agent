...
- ~~ADK interpolates `{ident}` / `{app:ident}` / `{user:ident}` placeholders in the system instruction from live session state on every request.~~ **Closed, and it was worse than latent** ([#1139](https://github.com/go-steer/core-agent/issues/1139)). "Nothing in-tree writes session state, so it's a no-op today" was the wrong reading of what an unwritten key does: ADK does not leave an unresolvable placeholder alone, it returns `ErrStateKeyNotExist` and fails the request, so an operator `AGENTS.md` containing a `{word}` did not make the system prompt per-request-variable — it ended every turn before a token was sent. Both `llmagent.New` sites now pass `InstructionProvider`, which substitutes nothing, so the system prompt is assembled once and is not a template. The cache consequence this bullet was filed for is settled in the direction it hoped for: the system block is request-invariant no matter what an operator writes in it.
