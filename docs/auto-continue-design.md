...
*Defect C (#796, v2.9):* the in-lifetime retry driver added by defect B turned a latent ambiguity in the classifier into a user-visible one. Detection reads committed history, where **an interrupted turn and a turn that is still running are the same tail** — the user's message with no model reply yet. On the boot path the ambiguity cannot arise (nothing is in flight after a restart), which is why it went unnoticed; on a timer-driven in-lifetime pass it is the common case. A live v2.9.0-dev daemon fronting a chat gateway declared a ~22-second generation interrupted 10 seconds in, queued a continuation note on the inbox, and ran it the instant the real turn finished: one user message, two replies. The run lock is not a defence here — it is fleet mutual exclusion, and the only local caller that takes it is `autonomous.Resume`, so an ordinary attach-driven or wake-loop turn holds no lease. The fix is a liveness check the injector consults before acting; see "In-flight turns are not interrupted turns" below.
...
From Alice's side the agent silently ignored her. Every deploy that catches a turn in flight produces one of these. The fix is a synthesized continuation turn: on resume, if the last turn was interrupted recently, run a turn whose prompt says "the previous turn did not complete — continue". (The note describes only what was *detected*, not a cause: interruption is inferred from tail shape, and the same note fires from the in-lifetime retry loop with no restart at all — so it must not claim a "daemon restart" it cannot verify. See #615.)
...
| Event carrying a **terminal** `ErrorCode` (safety, auth, malformed) | Turn ended with a failure the next attempt would reproduce — nothing to do |
| Event carrying a **transient** `ErrorCode` (`UNAVAILABLE`, `RESOURCE_EXHAUSTED`, `DEADLINE_EXCEEDED`, 429/502/503/504) | Provider blip — continue it, up to three times per interruption sequence (#969) |
...
One ambiguity: "user message with no model event" also matches *a user message that arrived while the daemon was down* (e.g. injected via a bridge that wrote directly to the DB). Both cases deserve a continuation turn, so the ambiguity is harmless. The *other* reading of that same shape is not — see "In-flight turns are not interrupted turns" below.

The one place tail *shape* is insufficient is the last row: a `MAX_TOKENS` truncation that still emitted text looks identical to a normal completion — both are a final model-text event. genai carries the distinction in `Candidate.FinishReason`, but ADK's storage row has no FinishReason column (`createEventFromStorageEvent` reconstructs the event without it), so a reloaded event can't see it. The fix (#582) piggy-backs on the one field ADK *does* round-trip — `CustomMetadata`: the eventlog overlay's `AppendEvent` stamps an *abnormal* finish reason into `CustomMetadata` under `FinishReasonMetadataKey` just before persist, and the classifier consults that stamp in the model-text arm. Only abnormal reasons are stamped (never the empty zero value or a plain `STOP`), so the ~99% normal-completion path is untouched and unstamped tails keep the "completed" default. Of the abnormal reasons, only `MAX_TOKENS` drives a continuation — the terminal-block reasons (`SAFETY`, `RECITATION`, `BLOCKLIST`, …) are recorded for audit but classify as completed, since a re-drive would only re-trigger the same block (they belong with the *terminal* `ErrorCode` finals as outcomes the user already saw). This mirrors the `CompactionMetadataKey` piggy-back the classifier already consults for compaction summaries.

*Implementation note (#969):* `ErrorCode` was originally read as one shape — any code, terminal — and that was wrong in the direction that costs the most. A safety block and a `503` under load are not the same event: the first will fail identically on the next attempt, the second will not, and for an unattended session declining on the second is not a degraded turn but a permanent stop with no operator-visible signal. The split delegates to `attach.ClassifyTurnError` so the runtime keeps one error vocabulary — the codes an operator reads on the wire as `transient_net` / `rate_limited` are exactly the ones the driver re-drives. It classifies the **code only**, never the accompanying `ErrorMessage`: that classifier is a substring scan built for provider prose, and pointing it at free-form model output is how a safety block quoting the words "rate limit" gets re-driven forever. A code the classifier does not recognize reads as terminal, which is the direction that is safe to be wrong in. `INVALID_ARGUMENT` deliberately stays terminal despite being the ambiguous case (#898) — deciding it is transient belongs at the model layer (#935), where the evidence is. Re-drives are bounded at three per interruption sequence, counted as committed continuation notes since the last human message, so the budget is reconstructed from history on every pass and survives the daemon restart an in-memory counter would not. The interval is the retry driver's own cadence (`retry_interval`, floored per session by the 10m `breakerWindow`); a second timer would only be a less-visible copy of it. When the budget runs out the daemon logs one line naming the count and the last code, because that is the one stand-down an operator cannot read off the transcript.

## In-flight turns are not interrupted turns (#796)

The table above reads tail *shape*, and shape cannot see time. "User message with no model event" is equally the signature of a turn that died and of a turn that is, right now, thirty seconds into composing a long answer. Detection has no way to tell them apart, because the difference is not recorded anywhere in the eventlog: the interruption timestamp is the *user* event's, so a freshness window sees a long generation as arbitrarily recent, and a "trailing user event newer than N seconds is probably still running" heuristic is a guess in a position where being wrong duplicates a reply to a human.

Liveness is a fact the running process holds, not one history records — so the injector asks for the fact. `Agent.TurnInFlight()` (exported for this in #796) reports whether a `Run` turn is executing on that agent; `lockClassifyInject` consults it and stands down with `acSkippedTurnInFlight`, which is refundable — no note was injected, and the turn commits its own reply. Three properties make it the right signal rather than a convenient one:

- **It brackets exactly the window that matters.** Registration (`setCancelInFlight`) happens in `Run` before the runner is handed the turn's message, and the clear happens in the post-drain cleanup after the last event has been yielded. Every event the *runner* writes for the turn — the opening user message included — is therefore written while `TurnInFlight` reports true, so the ambiguous tail can never be observed with the check reading false. (Events `Run` writes *before* registration are outside the bracket; today that is only #537's tail repair, which writes responses for a turn that was already interrupted.)
- **It is per-agent, hence per-session.** The reported daemon was multi-session; a per-process "is anything running" flag would have suppressed continuations for every other session while one was busy.
- **It is checked after classification, not before.** A turn that starts between the two reads is then caught by the check. The reverse order would clear the check and then classify the user message the new turn had just committed — the same bug, one instruction apart.

The scope is in-process, and deliberately so: a peer daemon's turn against a shared eventlog reports false here. That is not a regression — the run lock is fleet mutual exclusion between the callers that *take* it (`autonomous.Resume` and auto-continue itself), and a plain attach-driven or wake-loop turn takes no lease, so that case was unguarded before this change too — but it does mean the cross-daemon shape of this bug is still open. Closing it needs a durable in-flight signal (a lease taken for the duration of every turn, or a turn-start marker row); that is a real cost — a write on the hot path of every turn, plus a stale-lease story for turns killed by SIGKILL — and it is deferred until the shape is observed rather than hypothesised.

The fix is at the *caller*, not in the classifier, and that is the deliberate half of the choice: a pure function over `[]*session.Event` has no honest way to answer "is this still running", and giving it a clock-based approximation would replace a caller-side gap with a wrong answer everywhere. `ClassifyInterruptedTail`'s doc comment carries the limitation and names `Agent.TurnInFlight` as the fact to consult, so the next caller meets it at the classifier rather than in production.

## Continuation semantics

- **Prompt**: a synthesized system-note turn, not impersonated user text. It states what was *detected* (an unfinished turn), never a cause it cannot verify (#615):
  > `[system note] The previous turn did not complete (last committed at <ts>). The last committed events are in your history; any interrupted tool call has been answered with an interruption notice. Continue the task: re-issue interrupted tool calls if their results are still needed, then answer the user's outstanding message. If nothing remains to do, reply briefly to confirm.`
  >
  > The substring `previous turn did not complete` doubles as the loop-breaker marker the classifier greps for in committed history (one automatic attempt per interruption). The pre-#615 marker (`interrupted by a daemon restart`) is still recognized for one migration window so a note committed by an older binary isn't re-continued into a loop.
- **Re-issue nudge is scoped by tool classification (#624)**: the default note above tells the model it *may* re-issue interrupted tool calls "if their results are still needed." That imperative is correct for a mutating call (a half-finished write may need redoing) but harmful for a **read-only introspection** call — reflexively re-running a side-effect-free read is exactly what turned an operator's `stop` + `/interrupt` into a `list_agents` loop in the wild. So when the interrupted tail died *mid-tool* and **every** interrupted call classifies read-only (`tools.IsReadOnlyToolName`, the same `#460` read-only registry, not a note-local name list), the note drops the "re-issue interrupted tool calls" imperative and instead says the interrupted work had no side effects and to re-query *only* if the data is still needed. Any mutating or **unrecognized** interrupted call keeps the default note — fail-safe: an unclassifiable name keeps the nudge. The interrupted call names come from the classifier (`ClassifyInterruptedTailWithCalls`), which surfaces them only for the mid-tool shape (a bare unanswered user message or an already-committed tool response has none, and keeps the default note).
- **An operator's queued input outranks the note (#624)**: the note is injected into the same inbox operators type into, and a batch drains into one turn. If an operator has *already* queued input (the canonical case: they typed `stop`, then `/interrupt`) when auto-continue runs, injecting a "continue the task" note into that same batch lets the note outrank the stop. So the injector stands down (`acSkippedOperatorInput`) whenever `Agent.HasPendingOperatorInput()` reports a queued message from anyone other than auto-continue itself — the operator's message drives the next turn alone. This is a race-free complement to the classifier's interrupt-audit veto (which depends on the audit row being committed before the tail is re-read); the two together are why a deliberate stop is honored. The skip is refundable (no attempt was made).
...
- **A tick must not fire at a turn that is still going (#796).** Re-running the pass on a timer is the one thing that makes the classifier's shape-only verdict unsafe: a tick that lands *inside* a turn sees it as interrupted, because that is what an unanswered user message looks like. Exposure is roughly turn duration over interval — a turn longer than the interval is certain to be caught, but the reported case was a ~22-second generation — so a longer `retry_interval` is a mitigation, never a fix. The pass therefore checks `Agent.TurnInFlight()` before injecting; see "In-flight turns are not interrupted turns" above. The cadence itself is unchanged.
