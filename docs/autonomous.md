...
For long waits — human is on lunch; another agent has to finish first — the agent emits a *status* (a tool call that returns immediately) and ends its turn. A driver loop on the consumer side reads the next input from wherever (stdin, message queue, websocket) and starts a fresh `agent.Run`. Scion is the reference deployment: AGENTS.md tells the model to call `sciontool_status("ask_user", "...")` (a built-in tool in `pkg/tools`, auto-registered when `sciontool` is on PATH); the Scion harness shows the question to the human; their reply comes back via `scion message <agent>` → tmux send-keys → core-agent's TUI input → next `agent.Run`. See [Scion adapter]({{< relref "/docs/reference/scion-adapter.md" >}}).
