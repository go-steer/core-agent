...
**Status:** complete (2026-07-27). Human-led / API-shape; design-doc first per docs/cleanup-execution-plan.md (Wave 3). #388 completed first as planned (all four phases on main). PR 1 (`permissions.GrantStore` + gate wiring), PR 2 (`pkg/compose` skeleton: builders, formatters, pricing ops, log filter), PR 3 (`ConfigGrantStore` + persist helpers + main wiring), PR 4 (`runner.WakeLoop`, replacing main's `--no-repl` copy; the wake-loop-home open question below resolved in favor of `pkg/runner`), PR 5 (multi-session construction into compose; the per-session loop now runs on `runner.WakeLoop`), and PR 6 (`compose.AttachOptions` + `BuildAttachOptions`; main keeps the CLI-beats-config overlay) landed. All six PRs are on main — the extraction is done. PR 3 deviation from the sketch below: the store persists path grants as **typed `path_scope.allow_paths` entries carrying the grant's access** (widening in place on re-grant), not via bare `AppendPathScope` — bare `path_scope.allow` entries reload as `rw`, which would silently broaden a read-only grant on restart and defeat the gate's asymmetric promotion. The TUI's `AlwaysAllow` callback became a no-op (non-nil so the modal still offers the choice; the gate owns persistence). (Note: #492 later re-homed ConfigGrantStore to pkg/permissions and the persist helpers to pkg/config; compose keeps deprecated forwarders.)
