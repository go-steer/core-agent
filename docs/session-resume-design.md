...
- **Auto-recovering an in-progress turn at restart.** If the daemon crashed mid-turn, the in-flight tool call is lost. The session resumes to the state visible in the eventlog; whatever the agent was doing at the moment of crash is gone. (ADK's `session.Service` will naturally pick up at the last committed event.) Turn-boundary semantics for crash recovery is a substantive separate concern; defer.
  - *Implementation note (2026-07-30):* the corrupting half of this shipped as #537 — a crash/cancel between a persisted functionCall and its functionResponse used to leave a history every provider rejects. `Agent.Run` now repairs dangling tool-call tails at turn start by appending a synthesized error functionResponse (`pkg/agent/tail_repair.go`), so a resumed session is always well-formed. Auto-*continuing* the interrupted turn remains deferred, now tracked as #539 (design: `docs/auto-continue-design.md`).
