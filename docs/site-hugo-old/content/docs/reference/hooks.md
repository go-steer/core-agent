...
`core-agent` can shell out to operator-configured commands whenever the agent crosses a tool, model, or turn boundary. This is a general-purpose observation mechanism — Scion is one consumer ([Scion adapter]({{< relref "/docs/reference/scion-adapter.md" >}})), but any user can wire hooks to custom telemetry, notification services, project-specific triggers, or a local log tap.
...
- **Not a permission gate.** Hooks can't refuse a tool call or substitute a response. Use the [permissions gate]({{< relref "/docs/reference/permissions.md" >}}) for that.
- **Not a session archive.** For persistent event history use the [event log]({{< relref "/docs/reference/sessions.md" >}}); hooks are for out-of-process side effects.
