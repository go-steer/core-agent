...
**MCP servers are the integration point.** Anything outside the built-in catalog (files, search, shell, data + network, planning — see [Built-in tools]({{< relref "/docs/reference/tools.md" >}})) goes through MCP. Don't write custom Go tools unless you're embedding the library; for CLI use, MCP is the right surface.
