...
1. Scion launches `core-agent` inside a tmux session in a container. tmux gives it a real PTY, so core-agent's [TUI]({{< relref "/docs/reference/tui.md" >}}) auto-launches — `scion attach` shows the live TUI.
...
Each handler's command is passed to `/bin/sh -c` with the envelope JSON on stdin. Handlers run sequentially with a per-command timeout (default 10s; override via `timeout_seconds`). See [Hooks]({{< relref "/docs/reference/hooks.md" >}}) for the full mechanism.
...
Other env vars follow core-agent's [Providers]({{< relref "/docs/reference/providers.md" >}}) documentation.
...
- [Hooks]({{< relref "/docs/reference/hooks.md" >}}) — the general-purpose hook mechanism `pkg/hooks` exposes.
