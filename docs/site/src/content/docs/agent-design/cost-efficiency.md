...
For the mechanism details see [Context management](/concepts/context-management/) and [Configuration → pricing](/reference/configuration/).
...
core-agent --model claude-opus-5

# Explicit override is still supported (cross-provider, custom small model, etc.)
core-agent --model claude-opus-5 --agentic-small-model gemini-2.5-flash
```
...
- **Models row shows the Pro+Flash split is working** if Flash has a meaningful turn share. If Flash is 0 turns but you passed `--agentic-tools`, the model isn't routing through the wrappers — strengthen the `AGENTS.md` framing (see [Subagents and wrappers](/agent-design/subagents-and-wrappers/)).
...
- **[Context management](/concepts/context-management/)** — the underlying mechanisms; library API for `WithCompactor` / `WithCheckpointer`
- **[Subagents and wrappers](/agent-design/subagents-and-wrappers/)** — design patterns for `agentic_*` + background subagents
- **[System instructions](/agent-design/system-instructions/)** — `AGENTS.md` patterns including output-style rules
- **[Configuration → pricing](/reference/configuration/)** — `pricing.refresh`, `pricing.source`, per-model override; the layered catalog
- **[Providers](/concepts/providers/)** — model IDs, per-backend pricing notes
