...
It exists because prose cannot be cited. A parent that is required to ground its claims in evidence gets back a report, has nothing citable to point at, and re-issues the reads its child already did. Across the fifteen archived GKE drill runs that was 48% of everything the parent read after the handoff and 61% of the bytes it pulled back into its context, with two runs where *every* post-delegation read was a repeat.
