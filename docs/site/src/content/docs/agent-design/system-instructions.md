...
For the basic mechanics (file location, fallback chain, schema) see [Interactive quickstart → Step 1](/run/interactive/quickstart/). This page is the prescriptive how-do-I-write-a-good-one companion.
...
**Does NOT go in:** multi-step procedures, large reference content, per-task playbooks. Those go in [skills](/agent-design/skills/).
...
- **[Skills](/agent-design/skills/)** — when to write a skill vs. an `AGENTS.md` rule
- **[Subagents and wrappers](/agent-design/subagents-and-wrappers/)** — getting the model to use `agentic_*` tools + spawn background subagents
- **[Cost efficiency](/agent-design/cost-efficiency/)** — model selection, the Pro+Flash split, `/context` observability
- **[Interactive workflows](/run/interactive/workflows/)** — worked examples with full `AGENTS.md` files
- **[Reference → Skills](/concepts/skills/)** — `SKILL.md` schema details
