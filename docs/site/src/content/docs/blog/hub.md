...
- **End-to-end distributed tracing.** W3C `traceparent` propagates across the daemon, [`k8s-event-watcher`](https://github.com/go-steer/k8s-lookout), and every MCP tool call. The span waterfall (`mcp.tool_call → digest.process → subagent.llm_call`) is verified against [GKE Managed OpenTelemetry](https://docs.cloud.google.com/kubernetes-engine/docs/how-to/managed-otel-gke) landing in Cloud Trace. A `deploy/components/otel/` kustomize component makes the wiring a one-line overlay.
