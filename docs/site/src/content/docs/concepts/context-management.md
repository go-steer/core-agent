...
Five signals ship. Three are `Critical` — they halt under `enforce` and reach the model under `feedback`. Two are `Warn`: they never halt, in any mode. The line between them is what the detector can *prove*. The three Critical detectors all compare arguments, so when they fire the agent is demonstrably learning nothing. `repeated-tool-name` is a loop detector too, but it keys on the name alone and therefore cannot tell a loop from a sweep; halting a working agent is worse than being slow to stop a stuck one, so it warns.
...
- **It is a backstop, not the fix for that session.** Nine is under 15, and no threshold low enough to catch nine survives the budget above. The `mark_task_done` loop is fixed at the tool layer instead: a second call in the same turn now reports that the checkpoint was already recorded and that repeating cannot do anything further, rather than replying "acknowledged" again. A tool that cannot act twice in a turn should say so itself.
