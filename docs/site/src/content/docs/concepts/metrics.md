...
- `signal` — `repeated-tool-call` | `alternating-tool-cycle` | `dominant-tool-call` | `repeated-tool-name` | `tool-failure-streak` | `no-op-streak`. `severity` — `warn` | `critical`. See [autonomous operations](/run/autonomous/operations/) for what the watchdog does when it trips.
