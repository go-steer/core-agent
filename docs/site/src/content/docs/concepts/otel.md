...
- **Metrics are opt-in, on a separate pipeline from traces.** ADK-go still has no MeterProvider (upstream TODO), so the daemon builds its own: enable with `otel.metrics.exporter` in `.agents/config.json` or the `OTEL_METRICS_EXPORTER` env var (`otlp`, `prometheus` — scrape endpoint on `:9464` by default, override with `--metrics-addr` — or `both`). Today's surface: per-session GenAI token/cost meters, Go runtime metrics (`go.memory.*`, `go.goroutine.count`, GC config), two ADK-schema histograms — `gen_ai.agent.invocation.duration` (one point per turn — including subagent turns, which async/background subagents report under the bounded `gen_ai.agent.name=background_subagent`; `error.type` on failures) and `gen_ai.tool.execution.duration` (every tool call; includes mutation-lock and permission-prompt wait by design) — matching ADK Python/Kotlin dashboards, and the `core_agent.*` subsystem meters: digest calls/bytes-saved, compactions/checkpoints/subtasks/inbox per session, watchdog alerts by signal, autonomous runs by stop reason, attach sessions/subscribers/drops/peers, and MCP server status (`ok|error` — alert on the `error` dimension). The full instrument inventory lives in [`docs/metrics-design.md`](https://github.com/go-steer/core-agent/blob/main/docs/metrics-design.md).
