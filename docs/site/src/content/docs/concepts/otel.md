...
- **Metrics are opt-in, on a separate pipeline from traces.** ADK-go still has no MeterProvider (upstream TODO), so the daemon builds its own: enable with `otel.metrics.exporter` in `.agents/config.json` or the `OTEL_METRICS_EXPORTER` env var (`otlp`, `prometheus` — scrape endpoint on `:9464` by default, override with `--metrics-addr` — or `both`). Today's surface is per-session GenAI token/cost meters plus Go runtime metrics (`go.memory.*`, `go.goroutine.count`, GC config); the full instrument inventory and rollout plan live in [`docs/metrics-design.md`](https://github.com/go-steer/core-agent/blob/main/docs/metrics-design.md), with alignment to ADK's `gen_ai.*` histogram schema tracked in [#338](https://github.com/go-steer/core-agent/issues/338).
