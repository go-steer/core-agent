...
- **Metrics are opt-in, on a separate pipeline from traces.** ADK-go still has no MeterProvider (upstream TODO), so the daemon builds its own: enable with `otel.metrics.exporter` in `.agents/config.json` or the `OTEL_METRICS_EXPORTER` env var (`otlp`, `prometheus` — scrape endpoint on `:9464` by default, override with `--metrics-addr` — or `both`). Today's surface: per-session GenAI token/cost meters, Go runtime metrics (`go.memory.*`, `go.goroutine.count`, GC config), and two ADK-schema histograms — `gen_ai.agent.invocation.duration` (one point per turn, `error.type` on failures) and `gen_ai.tool.execution.duration` (every tool call; includes mutation-lock and permission-prompt wait by design) — matching ADK Python/Kotlin dashboards. The full instrument inventory lives in [`docs/metrics-design.md`](https://github.com/go-steer/core-agent/blob/main/docs/metrics-design.md); remaining subsystem meters are tracked in [#338](https://github.com/go-steer/core-agent/issues/338).
