...
**Three suppressed repeats end the turn (v3.0, [#1081](https://github.com/go-steer/core-agent/issues/1081)).** Not asking twice is cheaper than asking twice, but it is not an answer to a model that keeps going: re-running the rig above cost five fewer prompts and zero fewer calls, and the loop still ran until the [watchdog](/concepts/context-management/#watchdog-behavioral-observer--since-v25) counted to five and halted the session for the rest of its life. So after the gate has refused three calls in a turn without asking anybody, the **turn** ends. Three rather than the watchdog's five because the evidence is of a different quality — a repeated tool name cannot tell a loop from a sweep, whereas a suppressed repeat means the operator refused this exact request, the model was told so in a tool result, and it asked again — and counted across requests, so alternating between two already-refused calls is the same thing. What ends is only the turn: nothing is tripped, there is nothing to reset, and the next turn runs with the refusals in the model's history where they belong. The cut also takes those calls off the watchdog's books ([#1086](https://github.com/go-steer/core-agent/issues/1086)), because the watchdog is reading the same tool stream and its run length is not scoped to a turn — without that, the next turn's first identical call would be the fifth in a row and would halt the session, which is the outcome this exists to prevent, one turn later. That is safe rather than a hole: three suppressed repeats is a tighter bound than the watchdog's five, so a model looping through the gate is always stopped sooner than the watchdog would have stopped it, and a loop on a tool the gate never sees never reaches this arm at all. The cut appends an eventlog row (`Author=gate/refusal-storm`, carrying the count) and labels the turn's metric `refusal_storm`; it emits no `guardrail-trip`, because a client told about one would offer a reset for a guardrail that does not exist. It does write a `refusal_storm guardrail cut the turn in flight` line to the daemon log ([#1131](https://github.com/go-steer/core-agent/issues/1131)) — not a reversal of the previous sentence, since a log line has no reset vocabulary to misuse, and this is the one in-turn arm that reaches no live client at all, so before v2.10 an operator watching a gated run saw the turn die with a bare `context canceled` and nothing anywhere but the eventlog said why. See [Without an attach listener, the cut is in the log](/concepts/context-management/#without-an-attach-listener-the-cut-is-in-the-log). There is no flag to switch it off: the other in-turn guardrails are opt-in because they act on inference, and this one acts on a decision the operator already made.
