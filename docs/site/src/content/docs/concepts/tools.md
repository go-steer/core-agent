...
**A repeat is also visible to the watchdog** (v2.9, [#918](https://github.com/go-steer/core-agent/issues/918)). An `unchanged` result carries the reserved `no_op` key, so three identical `record_plan` calls in a row trip [`no-op-streak`](/concepts/context-management/#no-op-streak-v29) and halt the agent under `--watchdog=enforce`. `updated` does not: overwriting the artifact with a revision is work, and counting it would punish the in-place revision this guard exists to encourage. Note that both halves of this cross turn boundaries — an identical plan is `unchanged` in *any* later turn, and watchdog signal state is only cleared by an operator reset — so an agent that re-sends its plan verbatim on three consecutive turns halts too. Any productive tool result resets the run, so reaching three means the agent did nothing else in those turns. The `outcome` field is *not* what the detector reads — it is a three-value vocabulary this tool owns, for the model and the operator; the detector reads one reserved boolean, precisely so that rewording `outcome` cannot silently switch the detector off.
