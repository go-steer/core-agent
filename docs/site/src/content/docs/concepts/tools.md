...
- **Read-only by construction.** It refuses to poll anything not classified read-only — a loop that could call `write_file` sixty times is an amplifier, not a verifier. `wait_and_verify` itself and `ask_user` are refused unconditionally. An MCP tool is pollable when its server annotates it [`readOnlyHint`](/concepts/mcp/#read-only-tools-and-servers), or the server is declared `read_only: true` in `mcp.json`, or the tool is named in `poll_allow`.
...
This exists because a model that had drifted into completion-reporting mode called `record_plan` eight times in a single turn on a live cluster, seven of them consecutively, and minted `plan-5` through `plan-11` — each with a reworded plan, so [loop detection](/concepts/context-management/#watchdog-behavioral-observer--since-v25) saw eight distinct calls and nothing to trip on. The prose fix alone was already known not to work: the same shape on `mark_task_done` was answered with an honest "this did nothing" status and the model re-called thirteen times anyway. So the defence is the behaviour — no new file — and the message is secondary.

**A repeat is also visible to the watchdog** (v2.9, [#918](https://github.com/go-steer/core-agent/issues/918)). An `unchanged` result carries the reserved `no_op` key, so three identical `record_plan` calls in a row trip [`no-op-streak`](/concepts/context-management/#no-op-streak-v29) and stop the agent under `--watchdog=enforce`. `updated` does not: overwriting the artifact with a revision is work, and counting it would punish the in-place revision this guard exists to encourage. Note that both halves of this cross turn boundaries — an identical plan is `unchanged` in *any* later turn, and the signal's own streak survives a turn boundary — so an agent that re-sends its plan verbatim on three consecutive turns trips it too. Any productive tool result resets the run, so reaching three means the agent did nothing else in those turns. What the trip costs is [the turn, not the session](/concepts/context-management/#two-scopes-what-a-critical-is-entitled-to-stop) ([#1090](https://github.com/go-steer/core-agent/issues/1090)): the turn carrying the third call ends, the next one runs, and only three turns *cut this way* in a row halt the agent for an operator. The `outcome` field is *not* what the detector reads — it is a three-value vocabulary this tool owns, for the model and the operator; the detector reads one reserved boolean, precisely so that rewording `outcome` cannot silently switch the detector off.
...
**Withheld by `--checkpoint=operator`** (v2.9+, config `checkpoint.mode`, library `agent.WithoutMarkTaskDoneTool`). `/done`, the heuristic and `Agent.Checkpoint` all keep working; only the model's ability to declare a boundary goes away. This is the posture for a long-lived service — see [Choosing who declares a boundary](/concepts/context-management/) for the live failure it comes from, and note that it is the removal, not a stronger instruction, that fixes it: a tool description is prompt text the model reads at the moment of decision, and no amount of persona prose in the same build outranks it.
