...
Event-driven, **propose-only** K8s triage: the daemon plus the event-watcher sidecar and a triage skill that diagnoses through GKE's read-only MCP endpoint, verifies with `wait_and_verify`, proposes a fix, and pages on-call. It cannot mutate the cluster — enforced by the read-only endpoint, `tools.disable`, and `roles/container.viewer`, not by the persona. The watcher ships from [go-steer/k8s-lookout](https://github.com/go-steer/k8s-lookout) (`ghcr.io/go-steer/lookout`, pinned `v0.21.0` and deployed as `lookout-watch`); `v0.17.0` is the floor, since that release retired the `k8s-event-watcher` naming the manifests used to carry. **Minimum daemon image `2.9.0-dev.1`** — the config uses `alerts` and `tools.wait_and_verify`, and an older daemon drops both silently rather than failing ([#680](https://github.com/go-steer/core-agent/issues/680)).
