...
- **Guardrail halts**: the adapter projects the daemon's [`guardrail-trip` frame](/reference/attach-http/) (protocol 1.13.0) into `coretui.Event.GuardrailTrip`, and the TUI paints a `⚠ guardrail halted · watchdog` row with the daemon's reason under it, verbatim — the producer is the side that knows which reset clears the thing it just tripped, so the client invents no hint and offers no reset button. The frame's `halted_turn` arms a one-shot suppression of the `canceled` turn-error that an in-turn halt causes, which is why the version pairing matters in one direction: **a `core-agent-tui` older than core-tui v0.25.0, attached to a 1.13.0 daemon, loses the halt row and gains a contentless `⚠ canceled` block in its place.** That is a regression rather than the silent degradation `wake` and `pause` offer, and the daemon cannot mitigate it — not emitting the cancel is exactly what 1.13.0 stopped doing. The bundled client tracks the pin, so this only bites a host built against an older core-tui. In the other direction nothing is owed: against a pre-1.13.0 daemon no frame arrives and halts keep surfacing as `cost_ceiling` / `watchdog` turn-errors ([#891](https://github.com/go-steer/core-agent/issues/891)).
