...
| `anthropic.prompt_cache.enabled` | bool | `true` | `false` disables prompt caching for this project. The `--no-prompt-cache` CLI flag overrides this in the off direction; it cannot turn caching back on. A subagent with its own `model` block inherits this value unless it sets its own. No TTL field: Anthropic's 1-hour breakpoint bills writes at 2× input where the 5-minute one bills 1.25×, and the rate catalog carries a single write rate — see [#770](https://github.com/go-steer/core-agent/issues/770). |
