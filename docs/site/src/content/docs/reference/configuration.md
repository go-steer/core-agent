...
| `watchdog` | string | mode-dependent | Behavioral-watchdog posture: `off` (no observation), `warn` (observe + alert), `enforce` (observe + alert + halt until `/guardrail reset`). Unset resolves to **`enforce` for unattended runs** (`-p`, `--no-repl`, or a non-TTY stdin) and **`warn` for interactive REPL/TUI runs**. Set it here so a recipe ships its own backstop instead of relying on every invocation passing the flag. CLI: `--watchdog` (overrides this field). See [Context management → Watchdog](/concepts/context-management/#watchdog-behavioral-observer--since-v25). |
