...
- **What does not happen**: the daemon never auto-continues an interrupted interactive turn on its own. A session interrupted mid-question resumes with intact history but waits for the next message. (Opt-in auto-continue is a tracked design item, #539.)
