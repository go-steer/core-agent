...
- **Interrupted turns are finished automatically (on by default for daemons; #559)**: via [`agent.auto_continue`](/reference/configuration/#agentauto_continue), a session whose turn was cut off by the restart gets a synthesized "continue the task" turn — on first touch for attached sessions, and via a bounded boot-time scan for channel sessions nobody re-touches. The same detection also finishes a turn that stopped short because it hit the model's output-token cap mid-answer (a `MAX_TOKENS` truncation), picked up on the session's next touch or boot scan (#582). Guarded by a freshness window, a per-boot cap, a per-session attempt cap with in-lifetime self-heal (retries a transiently-failed continuation on a timer instead of waiting for a reboot; #575), and a crash-loop breaker (`agent_boot_log`); a self-heal that fires on the timer rather than at boot says so in the log — `auto-continue retry:`, not `auto-continue boot scan:`, so a recovery on a daemon that has been up for hours is not mistaken for a restart (#1066); deliberately-interrupted turns (`POST /interrupt`) are never resurrected, an operator message already queued when the restart is detected (e.g. a `stop`) drives the next turn on its own rather than competing with a continuation note, and an interrupted read-only introspection call is not blindly re-issued (#624). A turn that ended with a **model error** is judged on which error it was (#969): a safety block, an auth failure or a malformed request will fail identically on the next attempt and is left alone, while a transient one — `UNAVAILABLE`, `RESOURCE_EXHAUSTED`, `DEADLINE_EXCEEDED`, a 429/502/503/504 — is re-driven, because for an unattended session declining there is not a degraded turn but a permanent stop. That re-drive is capped at three per interruption sequence (a human message resets it) and the daemon logs one line naming the count and the last error code when it gives up, so a session that stops re-driving is distinguishable from one that finished. A session that is **mid-turn** is also left alone (#796): a turn still generating leaves the same committed tail as one that died — the user's message with no reply yet — so before acting the trigger asks the agent whether a turn is in flight. That matters for the in-lifetime retry above rather than for restarts: nothing is running just after a boot, but on a live daemon a retry tick that lands inside a turn would otherwise declare it interrupted and answer the same message twice — the longer the answer, the likelier a tick lands in it. The check covers turns *this process* is driving; a peer daemon's ordinary turn against a shared eventlog is not visible to it. A peer's *continuation* is handled separately and durably (#977): before injecting, a daemon records a claim on that exact interruption in the shared database, and a daemon that finds the interruption already claimed stands down instead of answering the same question twice. This is what makes a fleet of daemons sharing one Postgres eventlog safe here, where the run lock alone was not — the lock is released before the continuation turn runs, so without a claim the next daemon to look simply sees the same unanswered message and injects a second continuation. Claims expire after ten minutes so that a daemon killed before its continuation reaches the model does not strand the session, and the same claim stops a single daemon's own retry tick from re-injecting in that gap. Finally, a session whose [guardrail is tripped](/concepts/context-management/#resetting-a-tripped-guardrail) is left alone entirely (#1040): guardrail halts survive the restart that would otherwise have been the escape from them, a halted agent refuses turns before it reads its inbox, and so a continuation note queued into one is undeliverable by construction — it would just accumulate, one per retry tick, in a bounded queue that drops the operator's oldest messages to make room. The trigger folds the persisted halt in before asking, because on this exact path — a pod restarted mid-halt — no turn has run yet and the in-memory flags are still clear. The session continues the moment an operator resets the guardrail. Without any of this, a session interrupted mid-question resumes with intact history but waits for the next message.
