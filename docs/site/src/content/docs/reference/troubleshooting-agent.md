...
- **Provider-shaped alert templates** (`slack`, `discord`, `pagerduty_events_v2`). Designed in `docs/alert-tool-design.md`; rejected at config load until they ship. Use `generic` plus a receiver that fans out — or, for Slack and Google Chat, the [`switchboard`](/reference/configuration/#the-switchboard-template) template, which posts through the chat gateway instead of at the platform.
