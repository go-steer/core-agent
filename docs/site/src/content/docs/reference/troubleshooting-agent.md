...
From `2.9.0-dev.6`, the manifests themselves need the image. [#986](https://github.com/go-steer/core-agent/issues/986) dropped the `install-users-json` initContainer — a `runAsUser: 0` container in an otherwise non-root pod whose only job was one `chmod` — and mounts the Secret directly, which needs [#944](https://github.com/go-steer/core-agent/issues/944); and it swapped both probes to `httpGet /healthz`, which needs [#946](https://github.com/go-steer/core-agent/issues/946). Nothing checks that floor automatically, because it is a property of the YAML rather than of the config, but unlike the first one it fails loudly: an older daemon aborts at auth init on mode `0440`, and if it got past that it would 404 the probe and never go Ready.
:::
