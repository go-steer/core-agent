...
Budgets are evaluated between turns, with one exception: `WithMaxCost` is also checked **inside** a turn, after each model call. A single turn is an unbounded tool loop, so a between-turn-only spend cap sits idle through exactly the runaway it exists to stop. When the bound is crossed mid-turn the driver stops draining the turn, and the run ends with `max_cost_exceeded` carrying whatever text the model had produced up to that point. Every other cap is still evaluated at a turn boundary: a turn already in flight when one of those fires runs to completion (or to per-turn timeout) before the driver stops.

### Surviving the context wall on long runs

A long autonomous run that doesn't manage its context will eventually outgrow the model's window. The autonomous driver respects the same context-management mechanisms documented in [Context management](/concepts/context-management/) — if the agent was constructed with `WithCompactor(NewDefaultCompactor())` the post-turn threshold check fires between autonomous turns and the next turn drains a pending compaction before its actual work. Same for `WithCheckpointer` when the model calls `mark_task_done` between subgoals.
...
For unattended runs, use `permissions.ModeYolo` (or `ModeAllow` with an explicit allowlist) — `ModeAsk` would deadlock waiting for a human who isn't there. If you do use `ModeAsk`, wire a `permissions.Prompter` that fails fast (e.g. `tools.RefusePrompter`).
...
See [Permissions](/concepts/permissions/) for the underlying gate semantics.
...
See [Library API → Subagents](/embed/api/#subagents) for the full API surface — depth caps, custom branch labels, per-subagent options. `examples/with-subagent/` runs end-to-end with no credentials.
