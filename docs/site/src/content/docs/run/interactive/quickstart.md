...
> **Prefer to have an agent walk you through this?** The [`cli-setup` skill](/run/skills-library/cli-setup/) covers the same material in workflow form. Install once, then say "help me set up core-agent for my project" and the agent walks the four layers with you, writing files as you go.
...
You should have completed [Getting started](/run/getting-started/) — `go install`, environment credentials for a provider, and `core-agent -p "hello"` returning a response.
...
For deeper prompt-engineering patterns see [Agent design → System instructions](/agent-design/system-instructions/).
...
`-i` is mutually exclusive with `-p` (which runs one turn and exits) and incompatible with `--no-repl`. Works in both the TUI and the `--no-tui` line-mode REPL. For the slash command catalog see [Slash reference](/run/interactive/slash-reference/).
...
See the [Skills reference](/concepts/skills/) for the full `SKILL.md` schema, and [Agent design → Skills](/agent-design/skills/) for patterns on when to write a skill vs. an `AGENTS.md` rule.
...
With this, common workflows run without prompting; anything outside the allow-list still prompts. The pattern grammar (`*` = one segment, `**` = recursive) is documented in [Permissions](/concepts/permissions/).
...
- **[Workflows](/run/interactive/workflows/)** — the full worked example with MCP servers wired in, plus alternative agent shapes
- **[Slash reference](/run/interactive/slash-reference/)** — every slash command + keybinding
- **[Agent design](/agent-design/)** — prescriptive patterns: when to use skills vs. `AGENTS.md` rules, how to get the model to use subagents efficiently, cost-efficiency tips
- **[Context management](/concepts/context-management/)** — compaction, checkpoints, agentic tool wrappers for long sessions
- **[Configuration reference](/reference/configuration/)** — every field of `.agents/config.json`
- **[Built-in tools](/concepts/tools/)** — the model-facing tool catalog (file / search / shell / network / planning) and how to disable individual tools
- **[MCP servers](/concepts/mcp/)** — wire in third-party tools (web search, GitHub, databases)
- **[Autonomous quickstart](/run/autonomous/quickstart/)** — running unattended against a goal
