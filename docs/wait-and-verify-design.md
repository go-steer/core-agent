...
**Update ([#1098](https://github.com/go-steer/core-agent/issues/1098)).** The passthrough this section waited on arrived, from our side rather than ADK's: the adapter still drops `Annotations`, so `pkg/mcp` reads `readOnlyHint` off the `tools/list` response with client-side middleware and re-attaches it below the namespace wrap. A server that annotates its tools now classifies them itself, which is the case on the endpoints this recipe family actually mounts — all 15 tools on `container.googleapis.com/mcp/read-only` publish `readOnlyHint=true`, and all 23 on the full `/mcp` publish one either way. So `poll_allow` becomes an override rather than a requirement, as predicted above, and what is left for it is the case neither the annotation nor `read_only` reaches: a pollable tool on an *unannotated* server that also mutates.
