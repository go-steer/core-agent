...
1. **Phase 1** — `autonomous.RunAutonomous` capped at `MaxTurns(2)` to
   ...
   event log.
2. **Phase 2** — `autonomous.ResumeAutonomous` with a
...
- `autonomous.RunAutonomous` / `autonomous.ResumeAutonomous` / `autonomous.SessionRef` — `github.com/go-steer/core-agent/v2/pkg/agent/autonomous`
...
```
== Phase 1: RunAutonomous with MaxTurns(2) ==
[Phase 1] reason=max_turns_exceeded turns=2 done_detail="" final_text=""

== Phase 2: ResumeAutonomous picks up at the next turn ==
