...
operator (never checked in). Namespace: `gke-platform-agent`.
...
`scripts/gen-tokens.sh` does all of this for you, from `ADMIN_IDENTITY`
and `WATCHER_IDENTITY` in `scripts/prereqs.sh`. The manual steps below
are for deploying without the operator rig.
...
kubectl -n gke-platform-agent create secret generic core-agent-users \
...
kubectl -n gke-platform-agent create secret generic lookout-watch-token \
...
(`kubectl -n gke-platform-agent rollout restart deployment core-agent
lookout-watch`).
