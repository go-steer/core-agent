...
- the watcher (k8s-lookout's `lookout watch`, deployed as `lookout-watch`) runs with its default reason allow-list (`CrashLoopBackOff`, `ImagePullBackOff`, `ErrImagePull`, `OOMKilled`, `FailedMount`, `FailedScheduling`, `BackOff`, `Unhealthy`, `NetworkNotReady`, `NodeNotReady`, `Evicted`)
