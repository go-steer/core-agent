...
func AgenticReadFile(opts AgenticToolOpts) tool.Tool {
	...
	return agenticTool(opts,
		"agentic_read_file",
		"Read a file and return an authoritative digest answering your question. Use INSTEAD OF read_file (NOT IN ADDITION TO) when the file might be large and you only need a specific section (config files, manifests, long logs). Pass the file path AND what you're looking for as one combined request — e.g. 'read /etc/nginx.conf and tell me what listen ports are configured' or 'read internal/auth/handlers.go and summarize the login flow'. The subtask reads the raw file on your behalf, so the file's full content never enters your context window — you get the digest. Treat the digest as authoritative; DO NOT re-read the same path with bare read_file to spot-check or verify, that defeats the wrapper's purpose and re-introduces the raw content you were trying to avoid. If the digest doesn't cover something you need, call agentic_read_file again with a narrower question — refining beats double-reading.",
...
func AgenticFetchURL(opts AgenticToolOpts) tool.Tool {
	...
	return agenticTool(opts,
		"agentic_fetch_url",
		"Fetch a URL and return an authoritative digest answering your question. Use INSTEAD OF fetch_url (NOT IN ADDITION TO) when the page is likely long (documentation, articles, blog posts, search results) and you only need to answer a specific question from it. Pass the URL AND what you're looking for as one combined request — e.g. 'fetch https://example.com/api/auth and tell me what the bearer-token header format is'. The subtask reads the raw page on your behalf, so the full HTML never enters your context window — you get the digest. Treat the digest as authoritative; DO NOT re-fetch the same URL with bare fetch_url to spot-check or verify, that defeats the wrapper's purpose and re-introduces the raw HTML you were trying to avoid. If the digest doesn't cover something you need, call agentic_fetch_url again with a narrower question — refining beats double-fetching.",
...
func AgenticGrep(opts AgenticToolOpts) tool.Tool {
	...
	return agenticTool(opts,
		"agentic_grep",
		"Search files under a path for a pattern and return an authoritative ranked digest of the most relevant matches. Use INSTEAD OF grep (NOT IN ADDITION TO) when the pattern is likely to hit dozens of matches and you only need the top few (function definitions, config keys, error messages). Pass the pattern AND what you're looking for as one combined request — e.g. 'grep for TODO in internal/ and tell me which ones are about auth' or 'find where http.Server is constructed and which port it binds to'. The subtask runs grep on your behalf, so the raw match list never enters your context window — you get the digest. Treat the digest as authoritative; DO NOT re-run bare grep on the same pattern/files to spot-check or verify, that defeats the wrapper's purpose and re-introduces the raw match list you were trying to avoid. If the digest doesn't cover something you need, call agentic_grep again with a narrower question — refining beats double-grepping.",
		"You are a focused subtask. Your job: search files with grep, rank the matches by relevance to what the operator asked, and return a concise digest. Turn budget is tight (2 turns) — plan on a single grep call in turn 1 followed by the digest in turn 2; do NOT chase the matches with read_file unless the operator's question demands it (and even then prefer refusing and recommending a narrower follow-up call). Cite file:line verbatim from grep output for every match — do not paraphrase or invent paths. If you didn't read the file's surrounding lines, do NOT describe what's around the match; report only what grep actually returned. Stay under 500 words. If the pattern doesn't hit anything, say so directly.",
	)
}
...
func AgenticResearch(opts AgenticToolOpts) tool.Tool {
	...
	return agenticTool(opts,
		"agentic_research",
		"Investigate an open-ended question about the files or environment and return an authoritative structured walkthrough. Use INSTEAD OF (NOT IN ADDITION TO) hand-driving read_file/grep/list_dir/glob yourself, for 'understand how X works' or 'trace data flow from A to B' — questions that need multiple file reads + greps but whose intermediate exploration would clutter your context. Pass the question as the request. The subtask runs many read-only tools (read_file, grep, list_dir, glob) on your behalf and returns the walkthrough with file:line citations, so the raw tool output never enters your context window — you get the walkthrough. Treat the walkthrough as authoritative; DO NOT re-read or re-grep the cited locations with bare tools to spot-check or verify, that defeats the wrapper's purpose and re-introduces the raw content you were trying to avoid. If the walkthrough doesn't cover something you need, call agentic_research again with a narrower question — refining beats re-investigating manually.",
